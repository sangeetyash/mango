...
//	List<WebElement> searchFilters;
	...
	public void reportsModule() {
		try {
			log.info("Hitting card API and Fetching data from it's response");

			ReportsAPItest.cardAPITest(cardDetails);
	...
	public void realTimeMsgPage() {
		try {
			...
			log.info("user is redirected to card page successfully!!!!");
		} catch (Exception e) {
			Assert.fail("user is Not redirected to card page!!!!");
		}
	}
...
//			actions.sendKeys(Keys.PAGE_DOWN).build().perform();
			...
			accountNumber.sendKeys(cardDetails.get(1));
...
//			actions.sendKeys(searchFilters.get(4), accountDetails.get(5), Keys.ENTER).perform();
			...
			actions.sendKeys(searchFilters.get(5), cardDetails.get(6), Keys.ENTER).perform();

			log.info("Entering data in Account Holder ID filter");
			wait.until(ExpectedConditions.visibilityOf(accountNumber));
			accountHolderID.sendKeys(cardDetails.get(7));
...
//			d.manage().timeouts().pageLoadTimeout(30, TimeUnit.SECONDS);
			...
			Assert.fail("Search card Details via filters Test Failed!!!!");
		}
	}
	...
	public void verifyCardSearch() {
		...
		try {
			...
			if (spik.contentEquals(cardDetails.get(2)) && accountNum.contentEquals(cardDetails.get(1))
					&& processor.contentEquals(cardDetails.get(6))
					&& programName.contentEquals(cardDetails.get(3))) {

				log.info("Newly added card details are displayed in search result: ");
				...
				log.info("Newly added card details are not displayed in search result: ");
				Assert.fail("verify search card details result scenario failed");
			}
	...
	public void verifyCardResponse() {
		...
		try {
			...
			String responseUserID = d.findElement(By.xpath("(//*[@class='ant-table-row-cell-break-word'])[28]"))
					.getText();
			String responseUniqueID = d.findElement(By.xpath("(//*[@class='ant-table-row-cell-break-word'])[36]"))
					.getText();

			if (responseUserID.contentEquals(cardDetails.get(0))
					&& responseUniqueID.contentEquals(cardDetails.get(8))) {
				log.info("Newly added card details response is displayed in search result response: ");
				log.info("User ID: " + responseUserID);
				log.info("Unique ID: " + responseUniqueID);

			} else {
				log.info("Newly added card details response is not displayed in search result response: ");
				Assert.fail("verify search card details response result scenario failed");
			}
		} catch (Exception e) {
			Assert.fail("verify search card details response scenario failed");
